...
/* ✅ PASTE YOUR ENTIRE App CODE HERE (the full code from your canvas)
   BUT remove the line:
...
// ---- START: your code (edited only at the bottom export name) ----
...
// Per-person used question tracking
...
export default function App() {
  ...
  const [people, setPeople] = useState<Person[]>(() => {
    const loaded = loadJSON<Person[]>(LS.people);
    return Array.isArray(loaded) ? loaded : [];
  });

  const [activePersonId, setActivePersonId] = useState<string>(() => loadString(LS.activePersonId));

  const [questionIndex, setQuestionIndex] = useState<number>(() => {
    const week = currentWeekNumber();
    const stored = loadJSON<{ week: number; index: number }>(LS.questionState);
    if (stored && typeof stored.week === "number" && typeof stored.index === "number") {
      if (stored.week === week) return stored.index % QUESTIONS.length;
    }
  ...
  const [step, setStep] = useState<Step>(() => {
    ...
    const hasPeople = Array.isArray(loadedPeople) && loadedPeople.length > 0;
    return hasPeople ? "HOME" : "WELCOME";
  });

  const [lastSaved, setLastSaved] = useState<LastSaved | null>(null);
  const [toast, setToast] = useState<string>("");

  // Keep activePersonId valid
  useEffect(() => {
    if (people.length === 0) {
      setActivePersonId("");
      return;
  ...
  // Persist
  useEffect(() => saveJSON(LS.people, people), [people]);
  useEffect(() => saveString(LS.activePersonId, activePersonId), [activePersonId]);
  useEffect(() => {
    const week = currentWeekNumber();
          ...
          <div className="p-6">
            ...
            {step === "WELCOME" && (
              <div className="space-y-6 text-center">
                ...
                {people.length > 0 ? (
                  <SecondaryButton onClick={() => setStep("PEOPLE")}>Choose existing person</SecondaryButton>
                ) : null}
              </div>
            )}
...
// ... keep EVERYTHING from your canvas exactly the same ...

// 👇 ONLY change this last line:
// export default function App() {
export default function Page() {
  // keep your component body exactly the same
  // ...
}
// ---- END ----
