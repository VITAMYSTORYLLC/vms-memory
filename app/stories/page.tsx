...
export default function StoriesPage() {
    const { activeMemories, activePerson, lang, deleteMemory, setEditingId, setEditingPrompt, setStoryDraft, setImageDraft } = useMemory();
                ...
                <div className="p-4 pt-8 flex-1 flex flex-col h-screen sm:h-auto">
                    ...
                    <div className="flex-1 flex flex-col justify-center">
                        ...
                        <div className="mb-8 px-4 text-center">
                            <div className="text-[10px] font-bold uppercase tracking-[0.2em] text-stone-400 font-sans mb-2">
                                {activePerson ? "THE STORY OF" : "COLLECTION"}
                            </div>
                            <h1 className="text-3xl font-serif font-bold text-stone-900">{activePerson ? activePerson.name : "Stories"}</h1>
                            <p className="text-stone-500 text-sm mt-1">{activeMemories.length} stories collected</p>
                        </div>
                        ...
                        {activeMemories.length === 0 ? (
                            <div className="text-center p-8 border-2 border-dashed border-stone-200 rounded-2xl mx-4 bg-stone-50">
                                ...
                                <PrimaryButton onClick={() => router.push("/")}>Write First Story</PrimaryButton>
                            </div>
                        ) : (
                            <StoryCarousel
